...
public class HeroRequestConverter extends TwoWayConverter<HeroRequest, Hero> {
    ...
    public Hero convert(HeroRequest from) {
        return Hero.builder().name(from.getName()).heroCategories(from.getCategories().stream().map(cat -> new Category(cat,"")).collect(Collectors.toSet())).build();
    }

    @Override
    public HeroRequest convertBack(Hero from) {
        return HeroRequest.builder().name(from.getName()).build();
    }
}
