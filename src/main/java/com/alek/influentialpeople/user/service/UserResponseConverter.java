...
public class UserResponseConverter extends TwoWayConverter<User, UserResponse> {
    ...
    public UserResponse convert(User from) {
        return UserResponse.builder().username(from.getUsername()).email(from.getEmail()).roles(from.getRoles().stream().map(role -> role.getName()).collect(Collectors.toSet())).build();
    }
